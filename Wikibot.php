...
class Wikibot {
	...
	private function initCurl() {
		if ( !class_exists( 'Curl' ) || !class_exists( 'CurlResponse' ) )
			throw new Exception( "Failed to create Wikibot - could not find the Curl class" );
		
		$this->c              = new Curl();
		$this->c->user_agent  = "Wikibot 0.1";
		$this->c->cookie_file = "wikibot_cookie.txt";
	}
...
class WikiPage {
	 ...
	 */
	function __construct( $title, $wikibot ) {
	...
	function getText( $refresh = false ) {
		if ( $refresh ) { // we want to query the api
			...
			$r = $this->wikibot->query( $data ); // run the query
	...
	function setText( $text, $section = null, $minor = false, $summary = null ) {
		...
		$r = $this->wikibot->edit( $data ); // the edit query
	...
	function delete( $reason ) {
		...
		$r = $this->wikibot->delete( $data ); // the delete query
